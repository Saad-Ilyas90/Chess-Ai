...
class ChessBoardMultiplayer extends Component {
  ...
  componentDidMount() {
    ...
    if (window.chessAIApp && window.chessAIApp.setGameActive) {
      ...
      setTimeout(() => {
        alert(
          "⚠️ IMPORTANT: Using your browser's back button during this multiplayer game will forfeit your match. Your opponent will win automatically. Please use the in-game controls instead."
        );
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      prevProps.userColor !== this.props.userColor
    ) {
    ...
    return (
      <div
        className={`chess-board ${
          this.props.userColor === "b" ? "flipped" : ""
        }`}
      >
        {this.renderBoard()}
      </div>
